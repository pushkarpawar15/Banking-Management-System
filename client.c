...
int main()
{
    ...
    while(1)
    {
        if(i != 3) recv(sd, read_buffer, sizeof(read_buffer), 0); //Again logging in
        ...
        {
            if(login_user_send(sd)) i=i+1;
        }
        else if(i == 2) // output having sent the password
        {
            if(password_check(sd)) i=i+1;
        }
