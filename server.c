...
void stringReverse(char* begin, char* end) { // reverses a string
	char aux; 
	while(end>begin)
...
int closeConnection(int id) // this takes in an accunt_id and removes its value from the open_connections indicating that the account is now logging out
{
        ...
        else // main loop (can only be reached after a successful login)
        {
            while(1)
            {
                if(open_id != 1) // this is a user account (id == 1 has been reserved for the admin)
                {
                    ...
                    strcpy(write_buffer, "Enter 1 to Deposit\nEnter 2 to Withdraw\nEnter 3 for Balance Enquiry\nEnter 4 for Password Change\nEnter 5 to view details\nEnter 6 to exit\nEnter your option: ");
                    send(nsd, write_buffer, sizeof(write_buffer), MSG_CONFIRM);
                ...
                else if(open_id == 1) // admin
                {
                    ...
                    strcpy(write_buffer, "Enter 1 To Add an Account\nEnter 2 To Delete an Account\nEnter 3 To Get Account Details\nEnter 4 To Modify Account Password\nEnter 5 To Exit\nEnter 6 To Shut the Server Down\nEnter your option: ");
                    send(nsd, write_buffer, sizeof(write_buffer), MSG_CONFIRM);
